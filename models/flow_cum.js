...
class FlowCum{
	...
	static async all(){		
	    try {
	    	...
	    	await pool.connect();
	        let result = await pool.request().query('SELECT Flow_cum.*, Data_meta.projekt_name as projekt_name FROM Flow_cum LEFT JOIN Data_meta ON Flow_cum.data_meta_id=Data_meta.id');
	        pool.close();
	        if(result.recordset.length != 0){
	        	let returnArray = [];
	        	for(let r of result.recordset){
	        		returnArray.push(new FlowCum(r.id, r.date_time_for, r.value, r.data_meta_id, r.projekt_name));
	        	}
	        	return returnArray;
	        }
	        else
	...
	static async findById(id){
	    try {
	        ...
	        let result = await pool.request()
	            ...
	            .query('SELECT Flow_cum.*, Data_meta.projekt_name as projekt_name FROM Flow_cum '+
	            	'LEFT JOIN Data_meta ON Flow_cum.data_meta_id=Data_meta.id '+
	            	'WHERE Flow_cum.id = @input_parameter');
	        pool.close();
	        // console.log(result.recordset[0]);
	        if(result.recordset.length != 0)
	        	return new FlowCum(result.recordset[0]['id'], 
	...
	async save(){
		...
		try{
			...
			let result = await request.query('INSERT INTO Flow_cum (date_time_for, value, data_meta_id, createdAt, updatedAt) '
	...
	async update(){
		...
		try{
			...
			request.input('data_meta_id', sql.NVarChar, that.data_meta_id);
			request.input('updatedAt', sql.NVarChar, that.updatedAt);
			let result = await request.query('UPDATE Flow_cum SET '
