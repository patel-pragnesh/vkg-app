...
//        map.addLayer(dynamicMapServiceLayer);
        ...
        polygonSymbol = new esri.symbol.SimpleFillSymbol(esri.symbol.SimpleFillSymbol.STYLE_SOLID, 
            new esri.symbol.SimpleLineSymbol(esri.symbol.SimpleLineSymbol.STYLE_SOLID, new dojo.Color([255, 0, 0]), 4), 
            new dojo.Color([255, 255, 0, 0.03])
        );
...
//			map.graphics.clear();
			...
			pointSymbol.setOutline = new SimpleLineSymbol(SimpleLineSymbol.STYLE_SOLID, new esri_Color([255, 0, 0]), 1);
			pointSymbol.setSize(14);
			pointSymbol.setColor(new esri_Color([255, 0, 0, 0.5]));
...
//			Hernad_szelv_re(57487.5);	//		63064.7
            ...
            $("a[id^='directorate_link_']").click(function(){
                    VIZIG_hatar_ra($(this).attr('data-map'));
			});

            // Modellterület oldalon a szelvény választó változásakor térkép frissítés
            $('select[name=profile]').change(function(){
                var selectedProfile = $('select[name=profile] option:selected').text();
				$("div.profile_info").html('');
				Hernad_szelv_re(selectedProfile);
			});
        }

		function VIZIG_re(VIZIG_kod) {
			queryTask = new esri.tasks.QueryTask("https://geoportal.vizugy.hu/arcgis/rest/services/VKG/VKG_modellteruletek/MapServer/5");
			query = new esri.tasks.Query();
			query.returnGeometry = true;
			query.outFields = ["*"];
			query.where = "VIZIG_szam = " + VIZIG_kod;
			query.outSpatialReference = {"wkid": 3857};
			queryTask.execute(query, function(featureSet) {
				if (featureSet.features.length != 0) {
					map.graphics.clear();
					for (var i = 0; i < featureSet.features.length; i++) {
						var graphic = featureSet.features[i]; //Feature is a graphic
		...
		function VIZIG_hatar_ra(VIZIG_kod) {
			queryTask = new esri.tasks.QueryTask("https://geoportal.vizugy.hu/arcgis/rest/services/VKG/VKG_modellteruletek/MapServer/6");
			query = new esri.tasks.Query();
			query.returnGeometry = true;
			query.outFields = ["*"];
			query.where = "VIZIG_SZ_KOD = " + VIZIG_kod;
			query.outSpatialReference = {"wkid": 3857};
			queryTask.execute(query, function(featureSet) {
				if (featureSet.features.length != 0) {
					map.graphics.clear();
					...
					resultExtent = esri.graphicsExtent(featureSet.features).expand(1.4);
					map.setExtent(resultExtent,new esri.SpatialReference({ "wkid": 3857}));
				}
			});
		}

		function Modellter_re(Modellter_nev) {
			queryTask = new esri.tasks.QueryTask("https://geoportal.vizugy.hu/arcgis/rest/services/VKG/VKG_modellteruletek/MapServer/5");
			query = new esri.tasks.Query();
			query.returnGeometry = true;
			query.outFields = ["*"];
			query.where = "modellteru = '" + Modellter_nev + "'";
			query.outSpatialReference = {"wkid": 3857};
			queryTask.execute(query, function(featureSet) {
				if (featureSet.features.length != 0) {
					map.graphics.clear();
		...
		function Hernad_szelv_re(Hernad_szelv) {
			queryTask = new esri.tasks.QueryTask("https://geoportal.vizugy.hu/arcgis/rest/services/VKG/VKG_modellteruletek/MapServer/0");
			query = new esri.tasks.Query();
			query.returnGeometry = true;
			query.outFields = ["*"];
			query.where = "River_stat_n = " + Hernad_szelv;
			query.outSpatialReference = {"wkid": 3857};
			queryTask.execute(query, function(featureSet) {
				if (featureSet.features.length != 0) {
					map.graphics.clear();
					var graphic = featureSet.features[0]; //Feature is a graphic
					graphic.setSymbol(markerSymbol);
					map.graphics.add(graphic);
